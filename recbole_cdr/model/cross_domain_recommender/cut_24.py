...
class CCLLoss(nn.Module):
    ...
    def forward(self, pos_cos, neg_cos):
        ...
        return CCL_loss
class CUT_24(CrossDomainRecommender):
    ...
    def __init__(self, config, dataset):
        super(CUT_24, self).__init__(config, dataset)
        ...
        self.train_neg_sample_num = neg_sample_args.get('sample_num', neg_sample_args.get('by', 1))
    ...
    def target_train_forward(self, user, item, neg=True, train=1):
        if self.single_model_config['name'] == 'SimpleX':
    ...
    def predict(self, interaction):
        if self.phase == 'SOURCE':
            ...
            score = self.source_forward(user, item, 0, neg=False)
        elif self.phase == 'TARGET':
            user = interaction[self.TARGET_USER_ID]
            item = interaction[self.TARGET_ITEM_ID]
            score = self.target_train_forward(user, item, neg=False, train=0)
        else:
            user = interaction[self.TARGET_USER_ID]
            item = interaction[self.TARGET_ITEM_ID]
            score = self.target_forward(user, item, 0, neg=False)
